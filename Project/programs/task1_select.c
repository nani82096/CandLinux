				...
				if ( valread == 0)
                {
					...
					for ( j = 0; j <= max_sd; j++){
						...
						if ( flag == 0){
							...
							printf ("Message has been added to offline List\n");
							NODE *temp = head;
							while ( temp && temp->dst_ip != online_ip[j]){
								write (j, temp->buff, strlen(temp->buff));
								temp = temp->next;
							}
						}
