...
long myIoctl( struct file *fd, unsigned int pid, unsigned long var_for_walk )
{
	...
	printk("%7s%7s%7s%20s%20s\tcommand\n", "LWP", "PID", "PPID", "STIME", "TIME");
//	list_for_each_entry( all_task, &(my_task->tasks), tasks) { 
	list_for_each_entry( all_task, &(init_task.tasks), tasks) { 
		printk("%7d", all_task -> pid);
		printk("%7d", all_task -> tgid);
		printk("%7d", all_task -> parent -> pid);
		printk("%20llu", all_task -> start_time);
		printk("%20llu\t", all_task -> real_start_time);
		printk("%s\n", all_task -> comm);
	}

	//	printk("task_struct address from list entry is %p\n",all_task);
	printk("ioctl called is successful\n");
	return 0;
