...
void mypath(struct dentry *pptr)
{
	if ( pptr != pptr->d_parent ){
		mypath(pptr->d_parent);
		printk("/%s", pptr->d_name.name);
	}
}
//myIoctl: open ioctl 
int myioctl ( int fd, unsigned int cmd)
{
	struct task_struct *task1 = current;
	/*printing PID*/
	printk ("%d\n",task1->pid);
	struct vm_area_struct *temp = task1->mm->mmap;
	while ( temp){
		printk("%08x-%08x ",(int *)temp -> vm_start, (int *)temp -> vm_end);
		...
		printk("%08x ",temp -> vm_pgoff << PAGE_SHIFT);



		if (temp -> vm_file)
			printk(" %02u:",MAJOR(task1 -> mm  -> mmap ->vm_file -> f_path.dentry->d_sb->s_dev));
		else
			printk(" %02u:",0);

		if (temp -> vm_file)
			printk("%02u",MINOR(task1 -> mm  -> mmap ->vm_file -> f_path.dentry->d_sb->s_dev));
		else
			printk("%02u",0);

		if (temp -> vm_file)		
			printk(" %08lu",(temp -> vm_file -> f_inode -> i_ino));		
		else
			printk(" %u",0);
		printk(" ");
		if ( temp -> vm_file) 
			mypath(temp->vm_file->f_path.dentry);
			//printk(" %s",(temp -> vm_file -> f_path.dentry -> d_name.name));
