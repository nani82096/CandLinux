...
int main(int argc, char **argv)
{
    ...
    socklen_t serv_len;
    char buf[MAX] = {0};
    //char password[PASS_LIMIT] = "";
    char *passwd = (char *)malloc(sizeof(char)*1024);
    char *passwd1 = (char *)malloc(sizeof(char)*1024);
        ...
        if ( pid < 0 ) {
            ...
            while (pass_limit++ < P_LIMIT) {
                if ( recv(actfd, client_password, PASS_LIMIT, 0) == ERROR) 
                    handle_error("recv");
		strcpy(buff, (char *)crypt(client_password,key));
		//passwd = passwd - 7;
